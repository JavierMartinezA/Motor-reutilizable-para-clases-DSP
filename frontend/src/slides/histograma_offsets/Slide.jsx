    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.25fr', gap: 28, alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ background: '#08111d', borderRadius: 10, padding: '18px', color: '#e3d4ff', textAlign: 'center' }}>
            <MathFormula t="\Delta t_{\text{offset}} = t_{db} - t_{query}" color="#e3d4ff" size={1.3} display />
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ padding: '11px 16px', borderRadius: 8, background: '#eefcf2', borderLeft: `3px solid ${GREEN}`, fontFamily: "'Inter', sans-serif", fontSize: 14.5, color: INK }}>
              <strong style={{ color: GREEN }}>Correcta →</strong> pico estrecho y alto (delta de Dirac): alineación temporal.
            </div>
            <div style={{ padding: '11px 16px', borderRadius: 8, background: '#fbeeee', borderLeft: `3px solid ${RED}`, fontFamily: "'Inter', sans-serif", fontSize: 14.5, color: INK }}>
              <strong style={{ color: RED }}>Incorrecta →</strong> nube plana (ruido blanco): matches dispersos por azar.
            </div>
          </div>
          <div style={{ padding: '13px 18px', background: '#f3eefc', borderLeft: `3px solid ${VIOLET}`, borderRadius: 6, fontFamily: "'Newsreader', serif", fontSize: 16.5, fontStyle: 'italic', color: INK, lineHeight: 1.5 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: 4 }}>
            <div style={{ textAlign: 'center', padding: '14px 10px', borderRadius: 10, background: '#fbf9f5', border: '1px solid #e8e3d8' }}>
              <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 12.5, color: INK_MUTED, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700 }}>Margen · conteo simple</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 32, fontWeight: 800, color: INK_MUTED, marginTop: 4 }}>{fmt(data.simpleMargin)}</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12.5, color: INK_FAINT }}>{data.simpleA} vs {data.simpleB}</div>
            </div>
            <div style={{ textAlign: 'center', padding: '14px 10px', borderRadius: 10, background: '#f3eefc', border: `1px solid ${VIOLET}55` }}>
              <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 12.5, color: VIOLET, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700 }}>Margen · histograma</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 32, fontWeight: 800, color: VIOLET, marginTop: 4 }}>{fmt(data.histMargin)}</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12.5, color: INK_FAINT }}>{data.histA} vs {data.histB}</div>
            </div>
          </div>
          <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 14, color: INK_MUTED, textAlign: 'center', lineHeight: 1.5 }}>
